...
def home():
    return html.Div(children=[
        ...
        dbc.Row([
            dbc.Col([
                html.Label("Power Density W/m2:", style={'margin-left': '1em'}),
                dcc.Input(
                    id='power',
                    type='number',
                    value=0,
                    style={'margin-left': '0.9em', 'width': '5em'}
                )
            ]),
            dbc.Col([
                html.Label("Aw L/(m2.h.bar):", style={'margin-left': '1em'}),
                dcc.Input(
                    id='aw',
                    type='number',
                    value=0.6732,
                    style={'margin-left': '3.1em', 'width': '5em'}
                )
            ]),
            dbc.Col([
                html.Label("B L/(m2.h):", style={'margin-left': '1em'}),
                dcc.Input(
                    id='b',
                    type='number',
                    value=0.3996,
                    style={'margin-left': '6em', 'width': '5em'}
                )
            ]),
            dbc.Col([
                html.Label("S m:", style={'margin-left': '1em'}),
                dcc.Input(
                    id='s',
                    type='number',
                    value=0.000678,
                    style={'margin-left': '8.75em', 'width': '5em'}
                )
            ]),
...
def page_return(n_clicks, draw_type, feed_type, draw_flow_rate, feed_flow_rate, draw_concentrate_m,
                feed_concentrate_m, draw_temp_k, feed_temp_k, pi_bar, p_bar, jw, power, aw, b, s):
    if n_clicks != 0:
        ...
        df_power = df_test.drop(11, axis=1)
        df_aw = df_test.drop(12, axis=1)
        df_b = df_test.drop(13, axis=1)
        df_s = df_test.drop(14, axis=1)
